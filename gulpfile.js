...
////2.2 导入gulp-autoprefixer这个第三方模块  (添加前缀)
const autoprefixer = require('gulp-autoprefixer');

// 2.3 写一个打包css的方法
    const cssHandler =()=>{
        return gulp.src('./src/css/*.css')
        //找到src目录里面下的css目录下的所有后缀为.css的文件
        .pipe(autoprefixer())           //把css代码自动添加前缀
        .pipe(cssmin())   //压缩css代码
        .pipe(gulp.dest('./dist/css')) 
        //压缩完毕以后的css代码放在dist目录中的css文件夹里面
    }
//2.4 导出这个任务
...
const babel = require('gulp-babel');
        //是为了是js的es6语法转变为es5语法  但得在第三步里面的pipe(babel(添加配置))

//3.3 书写一个打包js的方法
const jsHandler = ()=>{
    return gulp.src('./src/js/*.js') 
    //找到src目录里面下的js目录下的所有后缀为.js的文件
    .pipe(babel({
        presets: ['@babel/env']
    }))    //转码es6转换成es5了,就可以压缩了
    .pipe(uglify())  //压缩
...
const htmlHandler = ()=>{
    ...
    //找到src目录里面下的pages目录下的所有后缀为.html的文件
    .pipe(htmlmin({
...
module.exports.font =fontHandler
...
//9.1 导入一个 gulp-webserver这个第三方模块
...
const serverHandler = ()=>{
    return gulp.src('./dist') //找到我要打开网页的文件夹，吧这个文件夹当做网站根目录
    .pipe(webserver({
        //需要一些配置项
...
module.exports.default = gulp.series(
    delHandler,
    gulp.parallel(cssHandler,jsHandler,htmlHandler,phpHandler,fontHandler,imgHandler,libHandler,sassHandler),
    // serverHandler,
    watchHandler
)
